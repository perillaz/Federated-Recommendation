...
from fr.model.DisenCDR import DisenCDR
...
class CrossTrainer(Trainer):
    ...
    def evaluate_embedding(self, current_source,source_UV, source_VU, target_UV, target_VU, source_adj=None, target_adj=None):
        self.source_user, self.source_item, self.target_user, self.target_item = self.model(current_source,source_UV, source_VU,
                                                                                            target_UV, target_VU)
    ...
    def reconstruct_graph(self, current_source,batch, source_UV, source_VU, target_UV, target_VU, source_adj=None, target_adj=None, epoch = 100):
        self.model.train()
        ...
        if epoch<10:
            self.source_user, self.source_item, self.target_user, self.target_item = self.model.wramup(current_source,source_UV,source_VU,target_UV,target_VU)
        else:
            self.source_user, self.source_item, self.target_user, self.target_item = self.model(current_source,source_UV,source_VU, target_UV,target_VU)
