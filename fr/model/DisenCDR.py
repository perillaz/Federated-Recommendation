...
from fr.model.singleVBGE import singleVBGE
from fr.model.crossVBGE import crossVBGE
...
class DisenCDR(nn.Module):
    def __init__(self, opt):
        ...
        self.source_user_embedding = {}
        self.source_item_embedding={}
        ...
        for i in clients_list:
            if i != target:
                source_num+=1
                self.source_user_embedding[source_num] = nn.Embedding(opt["source"+str(source_num)+"_user_num"], opt["feature_dim"])
                self.source_item_embedding[source_num] = nn.Embedding(opt["source"+str(source_num)+"_item_num"], opt["feature_dim"])
        self.source_user_embedding_share = nn.Embedding(opt["target_user_num"], opt["feature_dim"])
        ...
        self.user_index = torch.arange(0, self.opt["target_user_num"], 1)
        ...
        for i in clients_list:
            if i!= target:
                source_num+=1
                self.source_user_index[source_num] = torch.arange(0, self.opt["source"+str(source_num)+"_user_num"], 1)
                self.source_item_index[source_num] = torch.arange(0, self.opt["source"+str(source_num)+"_item_num"], 1)
        ...
        if self.opt["cuda"]:
            ...
            for i in range(source_num):
                self.source_user_index[i+1]=self.source_user_index[i+1].cuda()
                self.source_item_index[i+1]=self.source_item_index[i+1].cuda()
    ...
    def forward(self, current_source,source_UV, source_VU, target_UV, target_VU):
        self.current_source_user_embedding = self.source_user_embedding[current_source].cuda()
        self.current_source_item_embedding = self.source_item_embedding[current_source].cuda()
        self.current_source_user_index = self.source_user_index[current_source].cuda()
        self.current_source_item_index = self.source_item_index[current_source].cuda()
        source_user = self.current_source_user_embedding(self.current_source_user_index)
        target_user = self.target_user_embedding(self.target_user_index)
        source_item = self.current_source_item_embedding(self.current_source_item_index)
        target_item = self.target_item_embedding(self.target_item_index)
        source_user_share = self.source_user_embedding_share(self.current_source_user_index)
    ...
    def wramup(self, current_source,source_UV, source_VU, target_UV, target_VU):
        self.current_source_user_embedding=self.source_user_embedding[current_source].cuda()
        self.current_source_item_embedding=self.source_item_embedding[current_source].cuda()
        self.current_source_user_index=self.source_user_index[current_source].cuda()
        self.current_source_item_index=self.source_item_index[current_source].cuda()
